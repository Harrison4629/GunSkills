...
{
    ATTACK("attack", 0xAA0000),
    ROGUE("rogue", 0x00AAAA),
    SUPPORT("support", 0x00AA00),
    SCOUT("scout", 0xFFAA00);

    private final String name;
    private final MutableComponent display;
    private final MutableComponent helper;

    private ClassType(String name, int color)
    {
        this.name = name;
        Style style = Style.EMPTY.withColor(color);
        this.display = Component.translatable("class.gunskills." + name).withStyle(style);
        ...
        this.helper = this.display.copy().append(Component.translatable("class.gunskills." + name + ".helper")).withStyle(style);
    }
